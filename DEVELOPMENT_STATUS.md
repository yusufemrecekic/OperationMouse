...
- Manual camera review found a remaining few-frame behind-wall reveal during oblique wall-edge yaw. The project inherited UE 5.8's 10-uu perspective near clip while the 5-uu sphere sweep stopped its center at surface contact, allowing the near plane to begin beyond the occluder. A project-scoped 2-uu near clip plus configurable 2-uu sweep-contact padding now keeps the view origin/frustum on the visible side without enlarging the probe or changing passage geometry. Manual camera re-review remains required.
- The first soft-occluder hide/show experiment was manually rejected because the six independent chair pieces disappeared and restored separately. That implementation was removed. The calibration chair now remains rendered and explicitly ignores only `ECC_Camera`; Pawn/world/physics blocking is preserved, while walls, floors, roofs, passages and the table remain hard camera blockers. Final manual camera-through-chair and wall regression review remains required.
- Camera-Ignore chair visuals removed aggressive per-piece compression but allowed visible geometry to cut through the view. The calibration chair now uses two invisible query-only camera proxies for its broad seat/back masses; proxies block only `ECC_Camera`, legs remain porous, and all gameplay/physics collision stays unchanged. Editor build, targeted Scale validation and Map Check (0 errors / 0 warnings) PASSED; final manual camera review remains required.
