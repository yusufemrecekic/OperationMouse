...
- Candidate B wall-edge re-review confirmed the leak correction and generally acceptable close-space behavior. The remaining chair composition issue now has an opt-in `OMCameraSoftOccluder` foundation: only the six tagged chair pieces are skipped during bounded obstruction resolution and hidden per owning local player; structural geometry remains hard-blocking. Editor build, targeted Scale validation and Map Check (0 errors / 0 warnings) PASSED. Final manual chair/wall/under-table camera review remains required before camera acceptance.
